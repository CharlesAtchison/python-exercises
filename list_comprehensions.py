...
fruit_char_count = [len(fruit) for fruit in fruits]

# Exercise 9 - Make a variable named fruits_with_letter_a that contains a list of only the fruits that contain the letter "a"
fruits_with_letter_a = [fruit for fruit in fruits for char in fruit if char == 'a']
...
more_than_two_numerals = [num for num in numbers if len(str(num).replace('-', '')) >= 2]
...
def is_prime(num):
    ...
    if num in [2, 3]:
        return True
    if num > 1:
        #iterate through numbers to check for factors
